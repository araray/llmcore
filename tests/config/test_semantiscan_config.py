...
class TestSemantiscanConfigInDefaultTOML:
    ...
    def default_config_dict(self) -> Dict[str, Any]:
        ...
        if hasattr(importlib.resources, "files"):
            default_config_path = importlib.resources.files("llmcore.config").joinpath(
                "default_config.toml"
            )
            ...
            default_config_content = importlib.resources.read_text(
                "llmcore.config", "default_config.toml", encoding="utf-8"
            )
    ...
    def test_database_section_exists(self, default_config_dict):
        ...
        assert db_config["type"] == "chromadb", "database.type should be 'chromadb'"
    ...
    def test_metadata_store_section_exists(self, default_config_dict):
        ...
        required_fields = [
            ...
            "path",
            "table_name",
            "ingestion_log_table_name",
            "file_history_table_name",
        ]

        for field in required_fields:
            assert field in ms_config, f"metadata_store.{field} is missing"
    ...
    def test_embeddings_section_exists(self, default_config_dict):
        ...
        assert isinstance(emb_config["models"], dict), "embeddings.models must be a dictionary"
    ...
    def test_ingestion_section_exists(self, default_config_dict):
        ...
        required_git_fields = ["enabled", "default_ref", "ingestion_mode", "enable_commit_analysis"]
    ...
    def minimal_valid_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "chunking": {
                ...
                "strategies": {},
                "parameters": {
                    "RecursiveSplitter": {"chunk_size": 1000, "chunk_overlap": 150},
                    "LineSplitter": {"lines_per_chunk": 50},
                    "SubChunker": {"chunk_size": 500, "chunk_overlap": 50},
                },
            },
            "ingestion": {
                "embedding_workers": 4,
...
class TestSemantiscanConfigWithConfy:
    ...
    def test_config_loads_via_confy(self):
        ...
        # Load default config
        if hasattr(importlib.resources, "files"):
            default_config_path_obj = importlib.resources.files("llmcore.config").joinpath(
                "default_config.toml"
            )
