...
class TestContextInfoIntrospectionBasic:
    ...
    async def test_context_info_has_required_fields(self):
        ...
        try:
            ...
            assert hasattr(context_info, "context_truncation_applied"), (
                "Missing 'context_truncation_applied' field"
            )
            ...
            assert isinstance(context_info.completion_tokens, int), (
                "completion_tokens should be int"
            )
...
class TestRAGIntegration:
    ...
    async def test_rag_disabled_flag(self):
        ...
        try:
            await llm.chat(message="Test message", session_id="no_rag", enable_rag=False)

            context_info = llm.get_last_interaction_context_info("no_rag")

            assert context_info is not None
            assert context_info.rag_used is False, "rag_used should be False when enable_rag=False"
            assert (
                context_info.rag_documents_retrieved is None
                or context_info.rag_documents_retrieved == 0
    ...
    async def test_rag_enabled_flag(self):
        ...
        try:
            ...
            await llm.chat(message="Test query", session_id="with_rag", enable_rag=True)

            context_info = llm.get_last_interaction_context_info("with_rag")

            assert context_info is not None
            assert context_info.rag_used is True, "rag_used should be True when enable_rag=True"
    ...
    async def test_rag_documents_retrieved_count(self):
        ...
        try:
            # Add test documents
            await llm.add_documents_to_vector_store(
                [{"content": f"Document {i}", "metadata": {"id": i}} for i in range(5)]
            )

            # Chat with RAG, requesting top 3
            await llm.chat(
                message="Test query", session_id="rag_count", enable_rag=True, rag_retrieval_k=3
            )
    ...
    async def test_rag_documents_used_structure(self):
        ...
        try:
            ...
            # Chat with RAG
            await llm.chat(message="What is Python?", session_id="rag_structure", enable_rag=True)
...
class TestEdgeCases:
    ...
    async def test_very_long_session_id(self):
        ...
        try:
            ...
            assert context_info is not None, "Should handle long session IDs"
    ...
    async def test_special_characters_in_session_id(self):
        ...
        try:
            ...
            assert context_info is not None, "Should handle session IDs with special characters"
...
class TestIntegrationScenarios:
    ...
    async def test_typical_repl_workflow(self):
        ...
        try:
            ...
            interactions = ["Hello!", "What's 2+2?", "Tell me a joke", "Thanks!"]
    ...
    async def test_rag_workflow_with_context_tracking(self):
        ...
        try:
            ...
            # Query 1: Without RAG
            await llm.chat(message="What is Python?", session_id="rag_workflow", enable_rag=False)
            info_without_rag = llm.get_last_interaction_context_info("rag_workflow")

            # Query 2: With RAG
            await llm.chat(message="What is Python?", session_id="rag_workflow", enable_rag=True)
