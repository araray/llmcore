...
class GoogleAIEmbedding(BaseEmbeddingModel):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if task_type_str not in VALID_TASK_TYPES:
            logger.warning(
                f"Provided task_type '{task_type_str}' is not in the known valid list {VALID_TASK_TYPES}. "
        ...
        logger.info(
            f"GoogleAIEmbedding configured with model '{self._model_name}' and task_type '{self._task_type}'. "  # Log without prefix
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(
            f"Initializing Google Gen AI client for embeddings (model: {self._model_name})..."
        )
        try:
            client_options = {}
            if self._api_key:
            ...
            logger.info(
                f"GoogleAIEmbedding client initialized successfully for model '{self._model_name}'."
            )
        except Exception as e:
            logger.error(
                f"Failed to initialize Google Gen AI client for embeddings: {e}", exc_info=True
            )
    ...
    async def generate_embedding(self, text: str) -> list[float]:
        ...
        if not self._client or not genai_errors:
            raise EmbeddingError(
                model_name=self._model_name, message="Google Gen AI client/errors not available."
            )
        if not text:
        ...
        # --- End Add Prefix ---
        logger.debug(
            f"Generating Google AI embedding for single text (length: {len(text)}, model: {model_with_prefix}, task: {self._task_type})..."
        )
        try:
            ...
            if embedding_data:
                logger.debug(
                    f"Successfully generated Google AI embedding, dimension: {len(embedding_data)}."
                )
                return embedding_data
            else:
                logger.error(
                    f"Google AI embedding API returned no embedding data for model '{model_with_prefix}'. Response: {result}"
                )
                raise EmbeddingError(
            ...
            logger.error(
                f"Google AI API error during embedding generation (model: {model_with_prefix}): {e}",
                exc_info=True,
            )
            raise EmbeddingError(model_name=self._model_name, message=f"Google AI API Error: {e}")
        except Exception as e:  # Catch any other unexpected errors
            logger.error(
                f"Unexpected error generating Google AI embedding (model: {model_with_prefix}): {e}",
                exc_info=True,
            )
    ...
    async def generate_embeddings(self, texts: list[str]) -> list[list[float]]:
        ...
        for i, text_content in enumerate(texts):
            if not text_content:
                logger.warning(
                    f"Empty string found at index {i} in batch texts for Google AI. Skipping."
                )
            else:
        ...
        # --- End Add Prefix ---
        logger.debug(
            f"Generating Google AI embeddings for batch of {len(processed_texts)} non-empty texts (model: {model_with_prefix}, task: {self._task_type})..."
        )
        try:
            # Use the async client (client.aio)
            result = await self._client.aio.models.embed_content(
            ...
            if (
                embeddings_data
                and isinstance(embeddings_data, list)
                ...
                for emb in final_embeddings:
                    if emb is not None:
                        ...
                        output_embeddings.append(
                            [0.0] * dimension
                        )  # Use zero vector for empty inputs

                return output_embeddings
            else:
                logger.error(
                    f"Google AI embedding API returned mismatched data count or no data for model '{model_with_prefix}'. "
                    f"Expected {len(processed_texts)}, got {len(embeddings_data) if embeddings_data else 0}. Response: {result}"
                )
                raise EmbeddingError(
                    model_name=self._model_name,
                    message="API returned mismatched or no embedding data for batch.",
                )

        except genai_errors.GoogleAPIError as e:
            logger.error(
                f"Google AI API error during batch embedding generation (model: {model_with_prefix}): {e}",
                exc_info=True,
            )
            raise EmbeddingError(model_name=self._model_name, message=f"Google AI API Error: {e}")
        except Exception as e:
            logger.error(
                f"Unexpected error generating batch Google AI embeddings (model: {model_with_prefix}): {e}",
                exc_info=True,
            )
            raise EmbeddingError(
    ...
    async def close(self) -> None:
        ...
        """
        logger.debug(
            f"GoogleAIEmbedding for model '{self._model_name}' closed (no specific client cleanup typically needed)."
        )
