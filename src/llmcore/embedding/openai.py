...
class OpenAIEmbedding(BaseEmbeddingModel):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        embedding_specific_config = (
            config  # The manager passes the specific [embedding.openai] section
        )
        ...
        if not self._api_key:
            ...
            logger.debug(
                "API key not in [embedding.openai], will try main OpenAI provider or env var."
            )
        ...
        logger.info(
            f"OpenAIEmbedding configured with model '{self._model_name}'. "
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize AsyncOpenAI client for embeddings: {e}", exc_info=True
            )
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        if not self._client:
            raise EmbeddingError(
                ...
                message="OpenAI client not initialized. Call initialize() first.",
            )
        if not text:
        ...
        logger.debug(
            f"Generating OpenAI embedding for single text (length: {len(text_to_embed)}, model: {self._model_name})..."
        )
        try:
            ...
            if response.data and len(response.data) > 0:
                ...
                logger.debug(
                    f"Successfully generated OpenAI embedding, dimension: {len(embedding_data)}."
                )
                return embedding_data
            else:
                logger.error(
                    f"OpenAI embedding API returned no data for model '{self._model_name}'. Response: {response}"
                )
                raise EmbeddingError(
            ...
            logger.error(
                f"OpenAI API error during embedding generation (model: {self._model_name}): {e.status_code} - {e.message}",
                exc_info=True,
            )
            raise EmbeddingError(
                model_name=self._model_name,
            ...
            logger.error(
                f"Unexpected error generating OpenAI embedding (model: {self._model_name}): {e}",
                exc_info=True,
            )
    ...
    async def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.debug(
            f"Generating OpenAI embeddings for batch of {len(processed_texts)} texts (model: {self._model_name})..."
        )
        try:
            response = await self._client.embeddings.create(
                model=self._model_name, input=processed_texts
            )
            if response.data and len(response.data) == len(texts):
                embeddings_data = [item.embedding for item in response.data]
                logger.debug(
                    f"Successfully generated batch of {len(embeddings_data)} OpenAI embeddings."
                )
                return embeddings_data
            else:
                logger.error(
                    f"OpenAI embedding API returned mismatched data count or no data for model '{self._model_name}'. "
            ...
            logger.error(
                f"OpenAI API error during batch embedding generation (model: {self._model_name}): {e.status_code} - {e.message}",
                exc_info=True,
            )
            raise EmbeddingError(
                model_name=self._model_name,
            ...
            logger.error(
                f"Unexpected error generating batch OpenAI embeddings (model: {self._model_name}): {e}",
                exc_info=True,
            )
            raise EmbeddingError(
