...
class JsonSessionStorage(BaseSessionStorage):
    ...
    async def initialize(self, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                f"JSON context preset storage initialized at: {self._presets_dir.resolve()}"
            )
            logger.info(f"JSON episode storage initialized at: {self._episodes_dir.resolve()}")
        except OSError as e:
            logger.error(
                f"Failed to create JSON storage directories (main: {self._storage_dir}, presets: {self._presets_dir}, episodes: {self._episodes_dir}): {e}"
            )
    ...
    async def save_session(self, session: ChatSession) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Session '{session.id}' with {len(session.messages)} messages and {len(session.context_items)} context items saved to {session_file_path}"
            )
            ...
            logger.error(
                f"An unexpected error occurred while saving session '{session.id}': {e}",
                exc_info=True,
            )
    ...
    async def get_session(self, session_id: str) -> ChatSession | None:
        ...
        try:
            ...
            logger.debug(
                f"Session '{session_id}' loaded from {session_file_path} with {len(chat_session.messages)} messages and {len(chat_session.context_items)} context items."
            )
            return chat_session
            ...
            logger.error(
                f"An unexpected error or validation error occurred while loading session '{session_id}': {e}",
                exc_info=True,
            )
    ...
    async def list_sessions(self) -> list[dict[str, Any]]:
        ...
        try:
            if not await aios.path.isdir(self._storage_dir):
                logger.warning(
                    f"Storage directory {self._storage_dir} does not exist or is not a directory."
                )
                return []
            for filename in await aios.listdir(self._storage_dir):
                if (
                    filename.endswith(self._file_extension)
                    and not (self._storage_dir / filename).is_dir()
                    ...
                    try:
                        ...
                        if not session_id_from_file:
                            logger.warning(
                                f"Session file {filename} is missing an 'id' field. Skipping."
                            )
                            continue
                        metadata = {
                        ...
                        logger.warning(
                            f"Could not decode JSON from session file {filename}. Skipping."
                        )
                    except Exception as e:
        ...
        try:
            if await aios.path.exists(session_file_path):
                ...
                if await aios.path.exists(episode_file_path):
                    ...
                    logger.debug(
                        f"Episodes file for session '{session_id}' deleted from {episode_file_path}"
                    )

                return True
            else:
                logger.warning(
                    f"Attempted to delete non-existent session '{session_id}' at {session_file_path}"
                )
                return False
        except OSError as e:
            logger.error(
                f"Error deleting session file {session_file_path} for session '{session_id}': {e}"
            )
            raise SessionStorageError(f"Failed to delete session file for '{session_id}': {e}")
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while deleting session '{session_id}': {e}",
                exc_info=True,
            )
    ...
    async def save_context_preset(self, preset: ContextPreset) -> None:
        ...
        try:
            ...
            logger.error(
                f"An unexpected error occurred while saving preset '{preset.name}': {e}",
                exc_info=True,
            )
    ...
    async def get_context_preset(self, preset_name: str) -> ContextPreset | None:
        ...
        try:
            if not await aios.path.exists(preset_file_path):
                logger.debug(
                    f"Context preset file not found for name '{preset_name}' at {preset_file_path}"
                )
                return None
            ...
            logger.debug(
                f"Context preset '{preset_name}' loaded from {preset_file_path} with {len(context_preset.items)} items."
            )
            ...
            logger.error(
                f"An unexpected error or validation error occurred while loading preset '{preset_name}': {e}",
                exc_info=True,
            )
            raise StorageError(
    ...
    async def list_context_presets(self) -> list[dict[str, Any]]:
        ...
        try:
                ...
                if filename.endswith(self._file_extension):
                    ...
                    try:
                        ...
                        if not preset_name_from_file:
                            logger.warning(
                                f"Preset file {filename} is missing a 'name' field. Skipping."
                            )
                            continue
                        ...
                        logger.warning(
                            f"Could not decode JSON from preset file {filename}. Skipping."
                        )
                    except Exception as e:
            ...
            logger.debug(
                f"Found {len(preset_metadata_list)} context presets in {self._presets_dir}"
            )
    ...
    async def delete_context_preset(self, preset_name: str) -> bool:
        ...
        try:
            if await aios.path.exists(preset_file_path):
                ...
                logger.warning(
                    f"Attempted to delete non-existent context preset '{preset_name}' at {preset_file_path}"
                )
                return False
        except OSError as e:
            logger.error(
                f"Error deleting context preset file {preset_file_path} for preset '{preset_name}': {e}"
            )
            raise StorageError(f"Failed to delete preset file for '{preset_name}': {e}")
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while deleting preset '{preset_name}': {e}",
                exc_info=True,
            )
    ...
    async def rename_context_preset(self, old_name: str, new_name: str) -> bool:
        ...
        if old_name == new_name:
            logger.info(
                f"Attempted to rename preset '{old_name}' to the same name. No action taken."
            )
            return (
                True  # Or False, depending on desired idempotency semantics. True seems reasonable.
            )
        ...
        if await aios.path.exists(new_path):
            logger.warning(
                f"Cannot rename: preset with new name '{new_name}' already exists at {new_path}."
            )
            return False
        ...
        try:
            ...
            if not preset_to_rename:  # Should be caught by exists check, but defensive
                logger.error(
                    f"Preset '{old_name}' disappeared before rename operation could complete."
                )
                return False
            ...
            await self.save_context_preset(
                preset_to_rename
            )  # This uses preset_to_rename.name (which is new_name)
            ...
            logger.error(
                f"Storage error during rename of preset '{old_name}' to '{new_name}': {se}",
                exc_info=True,
            )
            ...
            if await aios.path.exists(new_path) and not await aios.path.exists(old_path):
                logger.warning(
                    f"Rename partially failed: new preset '{new_name}' saved, but old '{old_name}' might still exist if deletion failed after save."
                )
            raise  # Re-raise the original storage error
        except OSError as e:
            logger.error(
                f"OS error during rename of preset '{old_name}' to '{new_name}': {e}", exc_info=True
            )
            raise StorageError(f"OS error during preset rename: {e}")
        except Exception as e:
            logger.error(
                f"Unexpected error renaming preset '{old_name}' to '{new_name}': {e}", exc_info=True
            )
    ...
    async def add_episode(self, episode: Episode) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Episode '{episode.episode_id}' for session '{episode.session_id}' appended to {episode_file_path}"
            )
        except TypeError as e:
            ...
            logger.error(
                f"An unexpected error occurred while saving episode '{episode.episode_id}': {e}",
                exc_info=True,
            )
    ...
    async def get_episodes(
        ...
        try:
            if not await aios.path.exists(episode_file_path):
                logger.debug(
                    f"Episode file not found for session '{session_id}' at {episode_file_path}"
                )
                return []
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid JSON on line {line_num} in episode file for session '{session_id}': {e}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Error parsing episode on line {line_num} for session '{session_id}': {e}"
                    )
            ...
            logger.debug(
                f"Retrieved {len(paginated_episodes)} episodes for session '{session_id}' (offset={offset}, limit={limit})"
            )
            ...
            logger.error(
                f"An unexpected error occurred while loading episodes for session '{session_id}': {e}",
                exc_info=True,
            )
