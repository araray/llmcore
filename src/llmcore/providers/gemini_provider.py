...
try:
    ...
    from google.genai.errors import GoogleAPICallError, InvalidArgumentError
    google_genai_available = True
except ImportError:
    genai = None # type: ignore
    genai_types = None # type: ignore
    GoogleAPICallError = Exception # type: ignore
    InvalidArgumentError = Exception # type: ignore
...
class GeminiProvider(BaseProvider):
    ...
    """
    _client: Optional[Any] = None # Use Any for genai.GenerativeModel
    ...
    def __init__(self, config: Dict[str, Any], log_raw_payloads: bool = False):
        ...
        try:
            ...
            # Client is the model itself in google-genai
            self._client = genai.GenerativeModel(self.default_model)
            logger.info("Google Gen AI configured successfully.")
        except Exception as e:
            logger.error(f"Failed to configure Google Gen AI: {e}", exc_info=True)
    ...
    def _parse_safety_settings(self, settings_config: Optional[Dict[str, str]]) -> Optional[List[Dict[str, Any]]]:
        ...
        for key, value in settings_config.items():
            try:
                ...
                category = genai_types.HarmCategory[key.upper()]
                threshold = genai_types.HarmBlockThreshold[value.upper()]
    ...
    def _convert_llmcore_msgs_to_genai_contents(self, messages: List[Message]) -> List[Dict[str, Any]]:
        ...
        if processed_messages and processed_messages[0].role == LLMCoreRole.SYSTEM:
            ...
            # Note: For Gemini, system instructions are handled differently, passed to the model constructor.
            # Here we just extract it. The actual provider will need to handle it.
            # For the public API, it's often prepended as a user message.
            genai_history.append({"role": "user", "parts": [{"text": system_instruction_text}]})

        last_role = genai_history[-1]['role'] if genai_history else None
    ...
    async def chat_completion(
        ...
        **kwargs: Any
        ...
        model_name = model or self.default_model
        self._client.model_name = model_name # Update the model on the client instance
        ...
        genai_contents = self._convert_llmcore_msgs_to_genai_contents(context)
        if not genai_contents:
            raise ProviderError(self.get_name(), "No valid messages to send.")

        generation_config = genai_types.GenerationConfig(**kwargs) if kwargs else None

        tools_payload = [genai_types.FunctionDeclaration.from_dict(tool.model_dump()) for tool in tools] if tools else None
        ...
        except (GoogleAPICallError, InvalidArgumentError) as e:
    ...
    async def count_tokens(self, text: str, model: Optional[str] = None) -> int:
        ...
        try:
            response = await self._client.count_tokens_async(contents=[text], model=target_model)
    ...
    async def count_message_tokens(self, messages: List[Message], model: Optional[str] = None) -> int:
        ...
        target_model = model or self.default_model
        genai_contents = self._convert_llmcore_msgs_to_genai_contents(messages)
        try:
            response = await self._client.count_tokens_async(contents=genai_contents, model=target_model)
            return response.total_tokens
        except Exception as e:
