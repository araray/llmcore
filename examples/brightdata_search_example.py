...
- Provide your zone names (created at https://brightdata.com/cp/zones) either
  here via config_overrides (below) or in ~/.config/llmcore/config.toml:
...
async def main() -> None:
    ...
    overrides = {
        "search_providers": {
            "brightdata": {
                "serp_zone": os.environ.get("BRIGHTDATA_SERP_ZONE", "my_serp_zone"),
                "unlocker_zone": os.environ.get("BRIGHTDATA_UNLOCKER_ZONE", "my_unlocker_zone"),
            }
        }
    }
    ...
    try:
        ...
        if not available:
            logger.error("No search provider configured. Set BRIGHTDATA_API_TOKEN and try again.")
            return

        provider = llm.get_search_provider()
        logger.info("Default search provider: %s", provider.get_name())
        ...
        logger.info("\n--- scrape_url ---")
        scrape = await llm.scrape_url("https://example.com", response_format="raw")
        if scrape.success:
            logger.info(
        ...
        if datasets:
            ...
            meta = await llm.get_dataset_metadata(ds.id)
            if meta.fields:
